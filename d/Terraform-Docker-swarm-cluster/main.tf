...
resource "aws_instance" "swarm_master" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "chmod 600 /home/ubuntu/mykey.pem"
    ]
...
resource "null_resource" "swarm_join_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      "docker swarm join --token $(ssh -o StrictHostKeyChecking=no -i /home/ubuntu/mykey.pem ubuntu@${aws_instance.swarm_master[0].public_ip} 'docker swarm join-token worker -q') ${aws_instance.swarm_master[0].public_ip}:2377"
    ]
